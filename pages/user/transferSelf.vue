	...
	<view class="transferSub">
		<view class="content">
			<view class="title">
				...
				<view class="">
					<view class="">{{$t('wallet.menus2.text')}}</view>
					<view class="sub-title">{{$t('wallet.menus2.sub.text')}}</view>
					<view class="sub-title">{{$t('wallet.transfer.self.tips')}}</view>
				</view>
			</view>
		...
		methods: {
			...
			submit(){
				this.$refs.form.validate().then(res=>{
					...
					this.$http.post('/player/safe/toBalance',para,(res=>{
						if(res.code ==200){
							this.formData = {
								money :'',
								payPwd:''
