	...
	<view class="addWallet">
		<view class="content">
			<view class="title">
				...
				{{$t('page.addwallet.title')}}
			</view>
			<view class="form">
				<uni-forms ref="form" :modelValue="formData" :rules="rules" label-position="top">
					<uni-forms-item  name="account">
						<uni-easyinput type="text" v-model="formData.account" :placeholder="$t('wallet.form.realName')" />
					</uni-forms-item>
					<uni-forms-item  name="type">
						<view class="type-select">
							<uni-data-select  v-model="formData.type" :placeholder="$t('wallet.form.type')" :localdata="typeList" :clear="false"></uni-data-select>
						</view>
					</uni-forms-item>
					<uni-forms-item  name="address">
						<uni-easyinput type="text" v-model="formData.address" :placeholder="$t('wallet.form.walletId')" />
					</uni-forms-item>
		...
		data() {
			return {
				...
				rules: {
					account: {
						rules: [
							{required: true,errorMessage: this.$t('ruls.xxx.empty',{name:this.$t('wallet.form.realName')})}
						]
					},
					type: {
						rules: [
							{required: true,errorMessage: this.$t('ruls.xxx.empty',{name:this.$t('wallet.form.type')})},
						]
					},
					address: {
						rules: [
							{required: true,errorMessage: this.$t('ruls.xxx.empty',{name:this.$t('wallet.form.walletId')})},
							{
		...
		methods: {
			getWalletType(){
				...
				this.$http.get(url,res=>{
					if(res.code==200){
						let datas = res.data || []
						this.typeList = datas.map((item,index)=>{
							let para = {
								text:item,
								value:index
							}
			...
			submit(){
				this.$refs.form.validate().then(res=>{
					...
					para.type = this.typeList[this.formData.type]
					this.$http.post('/player/wallet',para,(res=>{
						if(res.code ==200){
							this.formData = {
								type:'',
								account :'',
